 ...
 */
public class PhotoContentJob extends JobService {
    ...
    static {
        JobInfo.Builder builder = new JobInfo.Builder(Constants.PHOTO_CONTENT_JOB_ID,
                new ComponentName("com.example.android.apis", PhotoContentJob.class.getName()));
    ...
    final Runnable mWorker = new Runnable() {
        @Override public void run() {
            scheduleJob(PhotoContentJob.this);
    ...
    public static boolean isScheduled(Context context) {
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N) {
            ...
            for (int i = 0; i < jobs.size(); i++) {
                if (jobs.get(i).getId() == Constants.PHOTO_CONTENT_JOB_ID) {
                    return true;
                }
            }
        }
    ...
    public static void cancelJob(Context context) {
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N) {
            JobScheduler js = context.getSystemService(JobScheduler.class);
            js.cancel(Constants.PHOTO_CONTENT_JOB_ID);
        }
    }
